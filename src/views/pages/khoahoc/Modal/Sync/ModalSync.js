...
import { getDevicesCourse } from "src/services/courseService";
...
const ModalData_synchronizing = (props) => {
  ...
  const changeCheck = (item, value) => {
    ...
    oldDevices[index].selected = value.target.checked;
  ...
  // }
  return (
    <CModal show={props.sync.syncRow.on_off} onClose={closeModal} size="lg">
                ...
                <CFormGroup row>
                  ...
                  <CCol>
                    <CInput
                      ...
                      onChange={(value) => props.sync.changeSyncRow(value)}
                    />
                  </CCol>
                </CFormGroup>
              </CCol>
        ...
        <CRow>
          ...
          <CCol xs="12" lg="4">
            <CCard className="card-sm h-100">
              <CCardHeader color="secondary">Gán 3/3 máy</CCardHeader>
              <CListGroup className="list-group-sm">
                {deviesInCourse.map((item) => {
                  return (
                    <CFade in={showCard} key={item.id}>
